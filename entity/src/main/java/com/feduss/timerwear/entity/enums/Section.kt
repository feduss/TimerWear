...
sealed class Section(val baseRoute: String, val parametricRoute: String = "") {
    ...
    data object Timer: Section("timer","timer/{${Params.WorkoutId.name}}?${Params.CurrentTimerIndex.name}={${Params.CurrentTimerIndex.name}}&${Params.CurrentRepetition.name}={${Params.CurrentRepetition.name}}&${Params.CurrentTimerSecondsRemaining.name}={${Params.CurrentTimerSecondsRemaining.name}}")
